...
const (
	driverName           = "securityCDN"
	defaultRootDirectory = "/var/lib/registry/security"
...
func init() {
	factory.Register(driverName, &securityCDNDriverFactory{})
}

// securityCDNDriverFactory implements the factory.StorageDriverFactory interface
type securityCDNDriverFactory struct{}

func (factory *securityCDNDriverFactory) Create(parameters map[string]interface{}) (storagedriver.StorageDriver, error) {
...
func (d *driver) PutContent(ctx context.Context, subPath string, contents []byte) error {
	...
	defer writer.Close()
	if d.enableSecurity {
		securityKey, ok := ctx.Value("securityKey").(string)
		if !ok {
			return errors.New("need securityKey in ctx when enable security")
		}
		nonce, ok := ctx.Value("nonce").([]byte)
		if !ok {
		...
		err = d.encrypt(writer, bytes.NewReader(contents), securityKey, nil)
		if err != nil {
			writer.Cancel()
			return err
		}
...
func (d *driver)encrypt(des io.Writer, src io.Reader, encryptKey string, nonce []byte) error {
	key, err := hex.DecodeString(encryptKey)
	if err != nil {
		return fmt.Errorf("hex decode string encryKey: %s  err: %v", encryptKey, err)
	}
	...
	desData := make([]byte, len(srcData))
	stream.XORKeyStream(desData, srcData)
	des.Write(desData)

	driverHmac := hmac.New(sha256.New, key)
	driverHmac.Write(desData)
	return nil
}